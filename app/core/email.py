"""Servicio de correo saliente usando la API HTTPS de Brevo.

Se usa una API HTTPS (https://api.brevo.com) en vez de SMTP porque hosts
...
A diferencia de Resend (que exige verificar un dominio completo por DNS
para salir de modo sandbox), Brevo permite enviar a cualquier destinatario
verificando solo la dirección remitente (un link de confirmación a esa
bandeja, sin tocar DNS) — útil cuando no se tiene un dominio propio.

Si `BREVO_API_KEY` no está configurada, el envío real se omite y solo se
...
def send_email(
    ...
    """Envía un correo (texto plano + versión HTML opcional) vía Brevo.
    ...
    if not settings.BREVO_API_KEY:
        logger.warning(
            "BREVO_API_KEY no configurada. No se envió correo real a %s. "
            "Asunto: %s | Cuerpo: %s",
    ...
    logger.info("Enviando correo a %s vía Brevo", to_email)
    try:
        response = httpx.post(
            BREVO_API_URL,
            json=payload,
        ...
        logger.info(
            "Correo enviado correctamente a %s (messageId=%s)",
            to_email, response.json().get("messageId"),
        )
    except httpx.HTTPStatusError as exc:
        logger.error(
            "Brevo rechazó el correo a %s (asunto=%s): %s %s",
